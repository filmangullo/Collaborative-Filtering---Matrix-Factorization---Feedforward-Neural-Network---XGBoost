...
# -----------------------------
def evaluate_model(matrix, similarity_df, threshold=1.0):
    actual, predicted = [], []
    actual_bin, pred_bin = [], []
...
if not unrated_predictions.empty:
    ...
    st.warning("Belum ada prediksi untuk ditampilkan untuk user ini.")

# Evaluasi model
st.subheader("📊 Evaluasi Model")
with st.spinner("Menghitung metrik evaluasi..."):
    mae, mse, rmse, r2, precision, recall = evaluate_model(
        user_item_matrix, user_similarity_df, threshold=4.0  # bisa diganti 4.0 jika mau dianggap 'like'
    )
...
st.metric("R²", f"{r2:.4f}")
st.metric("Precision (rating ≥ 4)", f"{precision:.4f}")
st.metric("Recall (rating ≥ 4)", f"{recall:.4f}")
